...
ratings = pd.read_table('./Data/ratings.dat',sep='::',engine='python',names=['userid', 'movieid', 'rating', 'timestamp'])
...
print 'split train & test set and construct graph '
...
print relation_train_positive.head()
print '----------------------------'
print relation_test.head()
print '----------------------------'
...
pd.DataFrame(all_user,columns=['user']).to_csv('./Data/all_user.csv',head = True , index = False)
pd.DataFrame(all_item,columns=['item']).to_csv('./Data/all_item.csv',head = True , index = False)
...
print 'negative_pairs construct ! '

# all_negative_pairs = pd.DataFrame(negative_pairs,columns=['user','item'])

# Save_DataFrame_csv(all_negative_pairs,'./Data/all_negative_pairs')

len_positive = len(relation_train_positive)
n = 4
len_negative = len_positive * n
...
print relation_train.head()
print '----------------------------'
print relation_train_label.head()
print '----------------------------'

print len(relation_train)
print len(relation_train_positive)
